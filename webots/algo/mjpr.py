...
class Actor(nn.Module):
    ...
    def __init__(self, obs_shape, action_shape, hidden_dim, encoder_type,
        ...
        self.image_encoder = make_encoder(encoder_type,
                                          ...
                                          num_filters,
                                          output_logits=True)
        self.laser_encoder = make_encoder('laser',
                                          obs_shape,
                                          encoder_feature_dim,
                                          num_layers,
                                          num_filters,
                                          output_logits=True)
        self.pos_encoder = make_encoder('pos',
                                        5,
                                        encoder_feature_dim,
                                        num_layers=2,
                                        num_filters=128,
        ...
        self.trunk = nn.Sequential(
            nn.Linear(self.image_encoder.feature_dim + self.laser_encoder.feature_dim + self.pos_encoder.feature_dim,
                      hidden_dim), nn.ReLU(),
    ...
    def forward(self,
        ...
        out2 = self.laser_encoder(obs[1], detach=detach_encoder)
        out3 = self.pos_encoder(obs[2], detach=detach_encoder)
        obs = torch.cat([out1, out2, out3], dim=-1)
...
class Critic(nn.Module):
    ...
    def __init__(self, obs_shape, action_shape, hidden_dim, encoder_type,
        ...
        self.pos_encoder = make_encoder('pos',
                                        ...
                                        output_logits=True)
        self.Q1 = QFunction(
            self.image_encoder.feature_dim + self.laser_encoder.feature_dim + self.pos_encoder.feature_dim,
            action_shape[0],
            hidden_dim)
        self.Q2 = QFunction(
            self.image_encoder.feature_dim + self.laser_encoder.feature_dim + self.pos_encoder.feature_dim,
...
class SPR(nn.Module):
    ...
    def __init__(self,
        ...
        self.laser_transition_model = make_transition_model(
            transition_model_type, encoder_feature_dim, action_shape,
            transition_model_layer_width)
        self.pos_transition_model = make_transition_model(
            transition_model_type, encoder_feature_dim, action_shape,
            transition_model_layer_width)
        self.fusion_transition_model = transformer()
        self.image_encoder = critic.image_encoder
        self.laser_encoder = critic.laser_encoder
        self.pos_encoder = critic.pos_encoder
        self.target_image_encoder = copy.deepcopy(critic.image_encoder)
        self.target_laser_encoder = copy.deepcopy(critic.laser_encoder)
        self.target_pos_encoder = copy.deepcopy(critic.pos_encoder)
        # image
        ...
        self.global_final_classifier_image = nn.Sequential(
            ...
            nn.Linear(latent_dim, encoder_feature_dim))
        # laser
        self.global_classifier_laser = nn.Sequential(
            nn.Linear(encoder_feature_dim, latent_dim), nn.ReLU(),
            nn.Linear(latent_dim, encoder_feature_dim))
        self.global_target_classifier_laser = nn.Sequential(
            nn.Linear(encoder_feature_dim, latent_dim), nn.ReLU(),
            nn.Linear(latent_dim, encoder_feature_dim))
        self.global_final_classifier_laser = nn.Sequential(
            nn.Linear(encoder_feature_dim, latent_dim), nn.ReLU(),
            nn.Linear(latent_dim, encoder_feature_dim))
        # pos
        self.global_classifier_pos = nn.Sequential(
            nn.Linear(encoder_feature_dim, latent_dim), nn.ReLU(),
            nn.Linear(latent_dim, encoder_feature_dim))
        self.global_target_classifier_pos = nn.Sequential(
            nn.Linear(encoder_feature_dim, latent_dim), nn.ReLU(),
            nn.Linear(latent_dim, encoder_feature_dim))
        self.global_final_classifier_pos = nn.Sequential(
            nn.Linear(encoder_feature_dim, latent_dim), nn.ReLU(),
            nn.Linear(latent_dim, encoder_feature_dim))
    ...
    def laser_spr_loss(self, latents, target_latents, observation):
        global_latents = self.global_classifier_laser(latents)  # proj
        global_latents = self.global_final_classifier_laser(global_latents)  # pred
        with torch.no_grad():
            global_targets = self.global_target_classifier_laser(target_latents)
        targets = global_targets.view(-1, observation.shape[1], self.jumps + 1,
                                      global_targets.shape[-1]).transpose(
            1, 2)
        latents = global_latents.view(-1, observation.shape[1], self.jumps + 1,
    ...
    def pos_spr_loss(self, latents, target_latents, observation):
        global_latents = self.global_classifier_pos(latents)  # proj
        global_latents = self.global_final_classifier_pos(global_latents)  # pred
        with torch.no_grad():
            global_targets = self.global_target_classifier_pos(target_latents)
        targets = global_targets.view(-1, observation.shape[1], self.jumps + 1,
                                      global_targets.shape[-1]).transpose(
            1, 2)
        latents = global_latents.view(-1, observation.shape[1], self.jumps + 1,
    ...
    def do_spr_loss(self, pred_latents_image, pred_latents_laser, pred_latents_pos, observation_image,
                    observation_laser, observation_pos):

        pred_latents_image = torch.stack(pred_latents_image, 1)
        pred_latents_laser = torch.stack(pred_latents_laser, 1)
        pred_latents_pos = torch.stack(pred_latents_pos, 1)
        image_latents = pred_latents_image[:observation_image.shape[1]].flatten(
            0, 1)  # batch*jumps, *
        laser_latents = pred_latents_laser[:observation_laser.shape[1]].flatten(
            0, 1)  # batch*jumps, *
        pos_latents = pred_latents_pos[:observation_pos.shape[1]].flatten(
            0, 1)  # batch*jumps, *
        neg_image_latents = pred_latents_image[observation_image.shape[1]:].flatten(0, 1)
        neg_laser_latents = pred_latents_laser[observation_laser.shape[1]:].flatten(0, 1)
        neg_pos_latents = pred_latents_pos[observation_pos.shape[1]:].flatten(0, 1)

        image_latents = torch.cat([image_latents, neg_image_latents], 0)
        laser_latents = torch.cat([laser_latents, neg_laser_latents], 0)
        pos_latents = torch.cat([pos_latents, neg_pos_latents], 0)

        observation_image = observation_image[self.time_offset:self.jumps +
                                                               self.time_offset + 1]
        observation_laser = observation_laser[self.time_offset:self.jumps +
                                                               self.time_offset + 1]
        observation_pos = observation_pos[self.time_offset:self.jumps +
                                                           self.time_offset + 1]
        target_images = observation_image.transpose(0, 1).flatten(2, 3)
        target_laser = observation_laser.transpose(0, 1).flatten(2, 3)

        target_pos = observation_pos.transpose(0, 1)
        target_images = self.transform(target_images, True)  # Augment
        target_laser = self.transform(target_laser, False)
        target_pos = self.transform(target_pos, False)
        with torch.no_grad():
            target_image_latents = self.target_image_encoder(target_images.flatten(0, 1))
            target_laser_latents = self.target_laser_encoder(target_laser.flatten(0, 1))
            target_pos_latents = self.target_pos_encoder(target_pos.flatten(0, 1))
        global_loss_image = self.image_spr_loss(image_latents, target_image_latents,
                                                observation_image)
        global_loss_laser = self.laser_spr_loss(laser_latents, target_laser_latents,
                                                observation_laser)

        global_loss_pos = self.pos_spr_loss(pos_latents, target_pos_latents,
                                              observation_pos)
        # split to batch, jumps
        spr_loss_image = global_loss_image.view(-1, observation_image.shape[1])
        spr_loss_depth = global_loss_laser.view(-1, observation_laser.shape[1])
        spr_loss_pos = global_loss_pos.view(-1, observation_pos.shape[1])
        return spr_loss_image, spr_loss_depth, spr_loss_pos
...
class mjpr(object):
    ...
    def __init__(
        ...
        self.actor.laser_encoder.copy_conv_weights_from(self.critic.laser_encoder)
        self.actor.pos_encoder.copy_conv_weights_from(self.critic.pos_encoder)
        self.log_alpha = torch.tensor(np.log(init_temperature)).to(device)
        self.log_alpha.requires_grad = True
        # set target entropy to -|A|
        self.target_entropy = -np.prod(action_shape)
        self.curl1 = CURL(50).to(device)
        self.curl2 = CURL(50).to(device)
        self.curl3 = CURL(50).to(device)
        # optimizers
        ...
        if self.encoder_type == 'pixel':
            ...
            self.image_encoder_optimizer = torch.optim.Adam(
                self.critic.image_encoder.parameters(), lr= encoder_lr*2)
            self.laser_encoder_optimizer = torch.optim.Adam(
                self.critic.laser_encoder.parameters(), lr= encoder_lr*2)
            self.pos_encoder_optimizer = torch.optim.Adam(
                self.critic.pos_encoder.parameters(), lr= encoder_lr*2)

            self.image_curl = torch.optim.Adam(
                self.critic.image_encoder.parameters(), lr=encoder_lr)
            self.laser_curl = torch.optim.Adam(
                self.critic.laser_encoder.parameters(), lr=encoder_lr)
            self.pos_curl = torch.optim.Adam(
                self.critic.pos_encoder.parameters(), lr=encoder_lr)
            ...
            self.awl_ = AutomaticWeightedLoss(num=3)
            self.awl_optimizer_ = torch.optim.Adam(self.awl_.parameters(),
                                                  lr=auxiliary_task_lr, weight_decay=0)
        self.cross_entropy_loss = nn.CrossEntropyLoss()
    ...
    def select_action(self, obs):
        with torch.no_grad():
            ...
            obs[2] = torch.FloatTensor(obs[2]).to(self.device)
            obs[0] = obs[0].unsqueeze(0)
            obs[1] = obs[1].unsqueeze(0)
            obs[2] = obs[2].unsqueeze(0)
    ...
    def sample_action(self, obs):
        ...
        with torch.no_grad():
            ...
            laser = torch.FloatTensor(obs[1]).to(self.device)
            pos = torch.FloatTensor(obs[2]).to(self.device)
            image = image.unsqueeze(0)
            laser = laser.unsqueeze(0)
            pos = pos.unsqueeze(0)
            mu, pi, _, _ = self.actor([image, laser, pos], compute_log_pi=False)
    ...
    def update_spr(self, spr_kwargs, L, step):
        ...
        observation_laser = spr_kwargs["laser"]
        observation_pos = spr_kwargs["pos"]
        ...
        input_obs = [observation_image[0].flatten(1, 2), observation_laser[0].flatten(1, 2),
                     observation_pos[0]]

        input_obs[0] = self.SPR.transform(input_obs[0], augment=True)
        #input_obs[1] = self.SPR.transform(input_obs[1], augment=True)
        #input_obs[2] = self.SPR.transform(input_obs[2], augment=True)
        # stem forward


        image_latent = self.SPR.image_encoder(input_obs[0])  # Fold if T dimension.
        laser_latent = self.SPR.laser_encoder(input_obs[1])
        pos_latent = self.SPR.pos_encoder(input_obs[2])

        cpc_loss1 = self.curl1.update_cpc(image_latent, laser_latent)
        cpc_loss2 = self.curl2.update_cpc(image_latent, pos_latent)
        cpc_loss3 = self.curl3.update_cpc(pos_latent, laser_latent)
        cpc_loss = self.awl_.forward(cpc_loss1,cpc_loss2,cpc_loss3)
        # spr loss
        pred_latents_image.append(image_latent)
        pred_latents_laser.append(laser_latent)
        pred_latents_pos.append(pos_latent)

        if self.jumps > 0:

            for j in range(1, self.jumps + 1):
                fusion_latent = torch.stack([image_latent, laser_latent, pos_latent])
                ...
                laser_fusion = fusion_latent[:, 1, :]
                pos_fusion = fusion_latent[:, 2, :]

                image_latent = self.SPR.image_transition_model.sample_prediction(
                    torch.cat([image_latent, image_fusion, action[j - 1]], dim=1))
                laser_latent = self.SPR.laser_transition_model.sample_prediction(
                    torch.cat([laser_latent, laser_fusion, action[j - 1]], dim=1))
                pos_latent = self.SPR.pos_transition_model.sample_prediction(
                    torch.cat([pos_latent, pos_fusion, action[j - 1]], dim=1))
                cpc_loss1 = self.curl1.update_cpc(image_latent, laser_latent)
                cpc_loss2 = self.curl2.update_cpc(image_latent, pos_latent)
                cpc_loss3 = self.curl3.update_cpc(pos_latent, laser_latent)
                cpc_loss = 0.1 * self.awl_.forward(cpc_loss1,cpc_loss2,cpc_loss3)+cpc_loss
                pred_latents_image.append(image_latent)
                pred_latents_laser.append(laser_latent)
                pred_latents_pos.append(pos_latent)
        spr_loss_image, spr_loss_laser, spr_loss_pos = self.SPR.do_spr_loss(pred_latents_image, pred_latents_laser,
                                                                            pred_latents_pos, observation_image,
                                                                            observation_laser, observation_pos)

        model_spr_loss_image = spr_loss_image[1:].mean(0)  # (jumps, bs) => (bs, )
        t0_spr_loss_image = spr_loss_image[0]
        model_spr_loss_laser = spr_loss_laser[1:].mean(0)  # (jumps, bs) => (bs, )
        t0_spr_loss_laser = spr_loss_laser[0]
        model_spr_loss_pos = spr_loss_pos[1:].mean(0)  # (jumps, bs) => (bs, )
        t0_spr_loss_pos = spr_loss_pos[0]
        final_spr_loss_image = self.t0_spr_loss_weight * t0_spr_loss_image.mean() + \
                               self.model_spr_weight * model_spr_loss_image.mean()
        final_spr_loss_laser = self.t0_spr_loss_weight * t0_spr_loss_laser.mean() + \
                               self.model_spr_weight * model_spr_loss_laser.mean()
        final_spr_loss_pos = self.t0_spr_loss_weight * t0_spr_loss_pos.mean() + \
                             self.model_spr_weight * model_spr_loss_pos.mean()

        all_loss=self.awl.forward(final_spr_loss_image,final_spr_loss_laser,final_spr_loss_pos)+0.05*cpc_loss



        self.image_encoder_optimizer.zero_grad()
        self.laser_encoder_optimizer.zero_grad()
        self.pos_encoder_optimizer.zero_grad()
        self.spr_optimizer.zero_grad()
        self.curl1.cpc_optimizer.zero_grad()
        self.curl2.cpc_optimizer.zero_grad()
        self.curl3.cpc_optimizer.zero_grad()
        self.awl_optimizer.zero_grad()
        self.awl_optimizer_.zero_grad()
        all_loss.backward()

        self.laser_encoder_optimizer.step()
        self.image_encoder_optimizer.step()
        self.pos_encoder_optimizer.step()
        self.curl1.cpc_optimizer.step()
        self.curl2.cpc_optimizer.step()
        self.curl3.cpc_optimizer.step()
        self.spr_optimizer.step()
        self.awl_optimizer.step()
        self.awl_optimizer_.step()
    ...
    def update(self, replay_buffer, L, step):
        ...
        if step % self.critic_target_update_freq == 0:
            ...
            soft_update_params(self.critic.pos_encoder,
                               self.critic_target.pos_encoder,
                               self.encoder_tau)
            soft_update_params(self.critic.laser_encoder,
                               self.critic_target.laser_encoder,
                               self.encoder_tau)
            ...
            soft_update_params(self.SPR.image_encoder,
                               self.SPR.target_image_encoder,
                               self.SPR.momentum_tau)
            soft_update_params(self.SPR.laser_encoder,
                               self.SPR.target_laser_encoder,
                               self.SPR.momentum_tau)
            soft_update_params(self.SPR.pos_encoder,
                               self.SPR.target_pos_encoder,
                               self.SPR.momentum_tau)
            soft_update_params(self.SPR.global_classifier_image,
                               self.SPR.global_target_classifier_image,
                               self.SPR.momentum_tau)
            soft_update_params(self.SPR.global_classifier_laser,
                               self.SPR.global_target_classifier_laser,
                               self.SPR.momentum_tau)
            soft_update_params(self.SPR.global_classifier_pos,
                               self.SPR.global_target_classifier_pos,
                               self.SPR.momentum_tau)
